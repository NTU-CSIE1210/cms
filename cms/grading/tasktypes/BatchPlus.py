...
class BatchPlus(TaskType):
    ...
    _FILES_COMPILE_TOGETHER = ParameterTypeString(
        "Files Compile Together",
        "files_compile_together",
        "")
    ...
    ACCEPTED_PARAMETERS = [_COMPILATION, _FILES_COMPILE_TOGETHER, _USE_FILE, _EVALUATION]
    ...
    def __init__(self, parameters):
        ...
        self.files_compile_together = []
        t = self.parameters[1]
        if t is not None:
            self.files_compile_together = t.split(", ")

        self.input_filename, self.output_filename = self.parameters[2]
        self.output_eval = self.parameters[3]
    ...
    def _executable_filename(codenames, language):
        ...
        return "exec_" + name + language.executable_extension
    ...
    def compile(self, job, file_cacher):
        ...
        for filename, manager in job.managers.items():
            if is_manager_for_compilation(filename, language):
                ...
                if filename in self.files_compile_together:
                    filenames_to_compile.append(filename)
